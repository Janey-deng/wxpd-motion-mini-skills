...
| 与 [.cursor/rules/motion.mdc](../.cursor/rules/motion.mdc) 关系 | 本目录是协议的**实例化产物**;motion.mdc 定义"该怎么做",本文件给"具体的 7 套答案" |
...
| C7 | 骨架屏切内容 | 内容容器 | 加载完成切换 | 渐显交叉 (crossfade) | 不适用 |
| C10 | 下拉刷新指示器 | 列表顶部 | 刷新数据 | 弹性下拉 | 回缩消失 |

> 编号沿用 v1 的 C1-C12 系列,v1.3 起仅保留 C1/C2/C3/C4/C5/C7/C10 共 7 个核心场景。
...
| 场景 | 入场 Skill |
| --- | --- |
| C1 居中弹窗 | [modal-scale-in-elegant](skills/modal-scale-in-elegant.md) |
...
| C7 骨架屏 | [skeleton-crossfade-in-elegant](skills/skeleton-crossfade-in-elegant.md) |
...
**7 个 skill** (全部入场, 全部 elegant) + 1 份目录文档 (本文件)。
...
场景缩写: modal / actionsheet / toast / card / popover
         skeleton / refresh

动作:    scale-in / slide-up / drop-in / stagger-in / anchor-in
         crossfade-in / pull-in
```

例:
- `modal-scale-in-elegant.md` = 居中弹窗 · 缩放浮入 · 优雅
